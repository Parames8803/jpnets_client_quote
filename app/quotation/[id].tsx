  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data: rawData, error: queryError } = await supabase
        .from('quotations')
        .select(`*, clients (*), quotation_rooms (room_total_price, rooms (*, products (*)))`)
        .eq('id', id_str)
        .single();
      ...
      const rooms = rawData.quotation_rooms
        .map((qr: any) => qr.rooms ? ({ ...qr.rooms, room_total_price: qr.room_total_price }) : null)
        .filter((room: any): room is Room & { products: Product[] } => room !== null);

      const allProducts = rooms.flatMap((room: Room & { products: Product[] }) =>
        room.products ? room.products.map((p: Product) => ({ ...p, room_type: room.room_type ?? 'General' })) : []
      );
...
const OverviewTab = ({ data, colors }: { data: QuotationDetailsData; colors: any }) => (
  <ScrollView contentContainerStyle={styles.tabContentContainer}>
    <InfoSection icon="person.fill" title="Client Information" colors={colors}>
      <InfoItem icon="person" label="Name" value={data.client.name || 'N/A'} colors={colors} />
      <InfoItem icon="phone" label="Contact" value={data.client.contact_number || 'N/A'} colors={colors} />
      <InfoItem icon="location" label="Address" value={data.client.address || 'N/A'} colors={colors} />
    </InfoSection>
    <InfoSection icon="doc.text.fill" title="Quotation Details" colors={colors}>
      <InfoItem icon="doc.text" label="Quotation ID" value={`#${data.quotation.id.slice(-8).toUpperCase()}`} colors={colors} />
      <InfoItem icon="calendar" label="Date Created" value={new Date(data.quotation.created_at).toLocaleDateString()} colors={colors} />
      <InfoItem icon="dollarsign" label="Total Value" value={`₹${data.quotation.total_price?.toFixed(2) || 'N/A'}`} colors={colors} />
    </InfoSection>
  </ScrollView>
);

const ProductsTab = ({ products, colors }: { products: (Product & { room_type?: string })[]; colors: any }) => (
  <ScrollView contentContainerStyle={styles.tabContentContainer}>
    {products.length > 0 ? (
      products.map((product, index) => (
        <ProductCard key={`${product.id}-${index}`} product={product} index={index} colors={colors} />
      ))
    ) : (
...
const RoomsTab = ({ rooms, colors }: { rooms: (Room & { products: Product[] })[]; colors: any }) => (
  <ScrollView contentContainerStyle={styles.tabContentContainer}>
    {rooms.length > 0 ? (
      rooms.map(room => (
        <InfoSection key={room.id} icon="house.fill" title={room.room_type || 'Uncategorized'} colors={colors}>
          {room.products.length > 0 ? (
            room.products.map((product, index) => (
              <ProductCard key={product.id} product={product} index={index} colors={colors} showRoomType={false} />
            ))
          ) : (
            <Text style={{ color: colors.secondaryText, padding: 10 }}>No products in this room.</Text>
          )}
        </InfoSection>
      ))
    ) : (
...
export default function QuotationDetailsScreen() {
  ...
  const handleDelete = () => Alert.alert('Delete Quotation', 'Are you sure?', [
    ...
    { text: 'Delete', style: 'destructive', onPress: async () => {
        ...
        try {
          ...
          router.replace(data?.client?.id ? `/client/${data.client.id}` : '/clients');
        } catch (err: any) {
  ...
  if (loading || isDeleting) {
    return (
      <View style={[styles.loadingContainer, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.loadingText, { color: colors.text }]}>{isDeleting ? 'Deleting...' : 'Loading...'}</Text>
      </View>
    );
  }
  ...
  if (error || !data) {
    return (
      <View style={[styles.loadingContainer, { backgroundColor: colors.background }]}>
        <Text style={[styles.errorText, { color: colors.error }]}>{error?.message || 'Quotation not found.'}</Text>
      </View>
    );
  }
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'overview': return <OverviewTab data={data} colors={colors} />;
      case 'products': return <ProductsTab products={data.allProducts} colors={colors} />;
      case 'rooms': return <RoomsTab rooms={data.rooms} colors={colors} />;
      default: return null;
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.tabBar, { borderBottomColor: colors.border }]}>
        <TabButton id="overview" title="Overview" activeTab={activeTab} onPress={setActiveTab} colors={colors} />
        <TabButton id="products" title="Products" activeTab={activeTab} onPress={setActiveTab} colors={colors} />
        <TabButton id="rooms" title="Rooms" activeTab={activeTab} onPress={setActiveTab} colors={colors} />
      </View>
...
const InfoItem = ({ icon, label, value, colors }: { icon: IconSymbolName; label: string; value: string; colors: any }) => (
  <View style={styles.infoItem}>
    <IconSymbol name={icon} size={16} color={colors.secondaryText} style={styles.infoItemIcon} />
...
const ProductCard = ({ product, index, colors, showRoomType = true }: { product: Product & { room_type?: string }, index: number, colors: any, showRoomType?: boolean }) => (
  <View style={[styles.productCard, { backgroundColor: colors.cardBackground, borderColor: colors.border }]}>
    <Text style={[styles.productName, { color: colors.text }]}>{product.name}</Text>
    {showRoomType && <Text style={[styles.productRoom, { color: colors.secondaryText }]}>{product.room_type}</Text>}
...
const TabButton = ({ id, title, activeTab, onPress, colors }: { id: Tab, title: string, activeTab: Tab, onPress: (id: Tab) => void, colors: any }) => {
  ...
  return (
    <TouchableOpacity onPress={() => onPress(id)} style={[styles.tabButton, isActive && { borderBottomColor: colors.tint }]}>
      <Text style={[styles.tabButtonText, { color: isActive ? colors.tint : colors.secondaryText }]}>{title}</Text>
    </TouchableOpacity>
  );
};
...
const generateQuotationHtml = ({ quotation, client, allProducts }: QuotationDetailsData): string => {
  /* The large HTML string for PDF generation remains the same as in the previous version. */
  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
const styles = StyleSheet.create({
  ...
  productMetaRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 },
  ...
  fab: { position: 'absolute', bottom: 30, right: 30, width: 60, height: 60, borderRadius: 30, justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 4, elevation: 8 },
});
