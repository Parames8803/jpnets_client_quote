...
export default function AddProductScreen() {
  ...
  const [products, setProducts] = useState<
    Omit<Product, "id" | "created_at" | "room_id">[]
  >([]);
  ...
  const [modalContent, setModalContent] = useState<
    "product" | "subProduct" | "productUnit" | null
  >(null);
  ...
  const handleSaveProducts = async () => {
    ...
    try {
      await supabase
        .from("products")
        .insert(products.map((p) => ({ ...p, room_id: room_id_str })));
    ...
    switch (modalContent) {
      case "product":
        ...
        onSelect = (product) => {
          ...
          setAvailableProductUnits(product.units || []);
      ...
      case "subProduct":
        ...
        options =
          selectedProducts.length > 0
            ? selectedProducts[selectedProducts.length - 1].sub_products || []
            : [];
        onSelect = (product) => {
      ...
      case "productUnit":
        ...
        onSelect = (option) => {
          setNewProductUnitType(option.name);
          setModalVisible(false);
          if (option.name.toLowerCase().includes("sq"))
          ...
          >
            ...
            {options.map((option, index) => (
              <TouchableOpacity
                ...
                style={[
                  styles.modalOption,
                  { backgroundColor: themedStyles.input.backgroundColor },
                ]}
                onPress={() => onSelect(option)}
              >
                <Text style={[styles.modalOptionText, themedStyles.text]}>
                  {option.name}
                </Text>
              </TouchableOpacity>
            ))}
  ...
  if (loading) {
    return (
      ...
      >
        ...
        <Text style={[themedStyles.text, { marginTop: 10 }]}>
          Loading room type...
        </Text>
      </View>
    );
  ...
  return (
    <KeyboardAvoidingView
      style={[styles.container, themedStyles.background]}
      behavior={Platform.OS === "ios" ? "padding" : "height"}
    >
      <StatusBar barStyle={isDark ? "light-content" : "dark-content"} />
      <View style={styles.header}>
        <Text style={[styles.headerTitle, themedStyles.text]}>
          Add Products to {roomType}
        </Text>
      </View>
      <ScrollView
      ...
      >
        <View style={[styles.card, themedStyles.card]}>
          <Text style={[styles.cardTitle, themedStyles.text]}>
            Products to Add
          </Text>
          {products.map((p, i) => (
            <View
              ...
              style={[
                styles.productItem,
                { borderBottomColor: themedStyles.border.borderColor },
              ]}
            >
          ...
          <View
            style={[
              ...
              { borderTopColor: themedStyles.border.borderColor },
            ]}
          >
            <TouchableOpacity
              style={[styles.selector, themedStyles.input]}
            ...
            <View style={{ flexDirection: "row", gap: 10 }}>
              ...
              />
              <TouchableOpacity
                style={[styles.selector, themedStyles.input, { flex: 1 }]}
                onPress={() => openModal("productUnit")}
              >
                <Text style={themedStyles.text}>
                  {newProductUnitType || "Unit"}
                </Text>
              </TouchableOpacity>
            </View>
            <TextInput
            ...
            >
              <TouchableOpacity onPress={resetProductForm}>
                <Text style={themedStyles.subtext}>Clear</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleAddProduct}>
                <Text style={themedStyles.primary}>Add to List</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <TouchableOpacity
          style={[
            styles.saveButton,
            { backgroundColor: themedStyles.primary.color },
          ]}
        ...
        <TouchableOpacity
          ...
          onPress={() => router.back()}
        >
          <Text style={[styles.cancelButtonText, themedStyles.subtext]}>
            Cancel
          </Text>
        </TouchableOpacity>
      </ScrollView>
